      ...
      <section className="space-y-6 pt-10">
        <div className="rounded-3xl border bg-card/50 p-6 md:p-10">
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant="secondary">operator-surface</Badge>
            <Badge variant="secondary">seasonal-agent</Badge>
          </div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground md:text-base">
            Sign in through SeasonalNet Auth to continue to the operator chat surface.
          </p>

          {error === "forbidden" ? (
            <div className="mt-4 rounded-xl border px-4 py-3 text-sm text-muted-foreground">
              Signed in as {sessionDisplayName(session)}, but this account is not authorized for the SeasonalNet Agent surface.
            </div>
          ) : null}
